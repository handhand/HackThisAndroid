...
static const char *libstocheck[NUM_LIBS] = {"libDetectFrida.so"};
...
static inline void parse_proc_maps_to_fetch_path(char **filepaths) {
    ...
    if ((fd = my_openat(AT_FDCWD, PROC_MAPS, O_RDONLY | O_CLOEXEC, 0)) != 0) {
        ...
        while ((read_one_line(fd, map, MAX_LINE)) > 0) {
            for (int i = 0; i < NUM_LIBS; i++) {
                __android_log_print(ANDROID_LOG_VERBOSE, "haha map line", "%s", map);
...
static inline bool fetch_checksum_of_library(const char *filePath, execSection **pTextSection) {
    ...
    for (int i = 0; i < ehdr.e_shnum; i++) {
        ...
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "a SectionHeader[%d][%u]", sectHdr.sh_name, sectHdr.sh_flags);

        //Typically PLT and Text Sections are executable sections which are protected
        if (sectHdr.sh_flags & SHF_EXECINSTR) {
            __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "b SectionHeader[%d][%u]", sectHdr.sh_name, sectHdr.sh_flags);

            offset[execSectionCount] = sectHdr.sh_offset;
            memsize[execSectionCount] = sectHdr.sh_size;
            __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "offset:[%x] size:[%x]", sectHdr.sh_offset, sectHdr.sh_size);
            execSectionCount++;
...
//        (*pTextSection)->checksum[i] = checksum(buffer, 10);
        free(buffer);
    ...
    if (buf[2] == 'x') {
        if (buf[0] == 'r') {
            ...
            for (int i = 0; i < pElfSectArr->execSectionCount; i++) {
                __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "checkpoint1: start:[%lx], offset:[%lx], memsize:[%lx], pElfSectArr->startAddrinMem:[%lx]", start, pElfSectArr->offset[i], pElfSectArr->memsize[i], pElfSectArr->startAddrinMem);
            ...
            for (int i = 0; i < pElfSectArr->execSectionCount; i++) {
                __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "checkpoint2: buffer:[%lx], offset:[%lx], memsize:[%lx], pElfSectArr->startAddrinMem:[%lx]", buffer, pElfSectArr->offset[i], pElfSectArr->memsize[i], pElfSectArr->startAddrinMem);
                unsigned long output = checksum(buffer + pElfSectArr->offset[i],
                                                pElfSectArr->memsize[i]);
//                unsigned long output = checksum(buffer + pElfSectArr->offset[i],10);
    ...
    if (dir != NULL) {
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            if ((filestat.st_mode & S_IFMT) == S_IFLNK) {
                ...
                __android_log_print(ANDROID_LOG_WARN, "haha named pipe",
                                    "%s", buf);
